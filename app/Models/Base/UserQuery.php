 ...
 *
 * @method     ChildUserQuery leftJoinBoard($relationAlias = null) Adds a LEFT JOIN clause to the query using the Board relation
 * @method     ChildUserQuery rightJoinBoard($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Board relation
 * @method     ChildUserQuery innerJoinBoard($relationAlias = null) Adds a INNER JOIN clause to the query using the Board relation
 *
 * @method     ChildUserQuery joinWithBoard($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Board relation
 *
 * @method     ChildUserQuery leftJoinWithBoard() Adds a LEFT JOIN clause and with to the query using the Board relation
 * @method     ChildUserQuery rightJoinWithBoard() Adds a RIGHT JOIN clause and with to the query using the Board relation
 * @method     ChildUserQuery innerJoinWithBoard() Adds a INNER JOIN clause and with to the query using the Board relation
 *
 * @method     \App\Models\BoardQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class UserQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'scv', $modelName = '\\App\\Models\\User', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, name, email, password, remember_token, created_at, updated_at FROM user WHERE id = :p0';
        try {
    ...
    /**
     * Filter the query by a related \App\Models\Board object
     *
     * @param \App\Models\Board|ObjectCollection $board the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
    ...
    public function filterByBoard($board, $comparison = null)
    {
        if ($board instanceof \App\Models\Board) {
            return $this
                ->addUsingAlias(UserTableMap::COL_ID, $board->getUserId(), $comparison);
        } elseif ($board instanceof ObjectCollection) {
            return $this
                ->useBoardQuery()
                ->filterByPrimaryKeys($board->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByBoard() only accepts arguments of type \App\Models\Board or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \App\Models\BoardQuery A secondary query class using the current class as primary query
     */
    public function useBoardQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinBoard($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Board', '\App\Models\BoardQuery');
    }
